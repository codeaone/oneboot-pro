...
import { Tooltip, Space, Input, ConfigProvider, Tabs,Tag } from 'antd';
...
function getSettingItem(setting: SettingPropType) {
  ...
  const ListToolBar: React.FC<ListToolBarProps> = ({
    ...
    const handleCloseTag =(tag:any) => {
      // console.log('+++++++++handleCloseTag+++++++++' + closeSearchTag);
      
      if(closeSearchTag) {
        closeSearchTag(tag.name);
      }
    }

    const renderSearchTag =()=> {
      console.log(searchItem);
    return  searchItem?.map(item=> <Tag key={item.label} closable onClose={()=>handleCloseTag(item)}>{`${item.label}:${item.value}`}</Tag>)
    }
