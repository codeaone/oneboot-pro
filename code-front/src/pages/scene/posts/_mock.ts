...
db.defaults({ sceneposts: d.data }).write();
...
export default {
  'POST /api/scene/posts': (req: Request, res: Response) => {
    //新增
    post(res, genData(req.body), tableName);
  },
  //删除数据
  'DELETE /api/scene/posts/:id': (req: Request, res: Response) => {
    deleteObj(res, req.params, tableName);
  },
  //修改数据
  'PATCH /api/scene/posts/:id': (req: Request, res: Response) => {
    patch(res, req.params, req.body, tableName);
  },
  //初始化数据
  'GET /api/scene/posts/init': {
    success: true,
    data: {
      state: state,
      initData: { state: '1' },
  ...
  //查看详情
  'GET /api/scene/posts/:id': (req: Request, res: Response) => {
    // const { id } = req.params;
    get(res, req.params, tableName);
  },
  //编辑时返回
  'GET /api/scene/posts/:id/edit': (req: Request, res: Response) => {
    get(res, req.params, tableName);
  },
  //列表
  'GET /api/scene/posts': (req: Request, res: Response) => {
