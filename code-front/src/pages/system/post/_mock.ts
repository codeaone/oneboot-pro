...
db.defaults({ systempost: d.data }).write();
...
export default {
  ...
  //修改数据
  'PATCH /api/system/post/:id': (req: Request, res: Response) => {
    patch(res, req.params, req.body, tableName);
  },
  //初始化数据
  'GET /api/system/post/init': {
    success: true,
    data: {
      state: state,
      initData: { state: '1' },
  ...
  //编辑时返回
  'GET /api/system/post/:id/edit': (req: Request, res: Response) => {
    get(res, req.params, tableName);
  },
  //列表
