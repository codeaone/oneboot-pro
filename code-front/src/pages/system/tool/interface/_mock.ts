...
const tableName = 'systeminterface';
...
db.defaults({ systeminterface: d.data }).write();
...
export default {
  'POST /api/system/interface': (req: Request, res: Response) => {
    //新增
    post(res, genData(req.body), tableName);
  },
  //删除数据
  'DELETE /api/system/interface/:id': (req: Request, res: Response) => {
    deleteObj(res, req.params, tableName);
  },
  //修改数据
  'PATCH /api/system/interface/:id': (req: Request, res: Response) => {
    patch(res, req.params, req.body, tableName);
  },
  //初始化数据
  'GET /api/system/interface/init': {
    success: true,
    data: {
      state: state,
      initData: { state: '1' },
  ...
  //查看详情
  'GET /api/system/interface/:id': (req: Request, res: Response) => {
    // const { id } = req.params;
    get(res, req.params, tableName);
  },
  //编辑时返回
  'GET /api/system/interface/:id/edit': (req: Request, res: Response) => {
    get(res, req.params, tableName);
  },
  //列表
  'GET /api/system/interface': (req: Request, res: Response) => {
